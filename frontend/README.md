...
| [Headwind](https://marketplace.visualstudio.com/items?itemName=heybourn.headwind) | Auto-sorts Tailwind classes into a consistent order. *(Skip if the repo's Prettier config already uses `prettier-plugin-tailwindcss` — same job, don't run both.)* |
